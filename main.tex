...
on remote servers with interfaces exposed on the internet particular focus to those with
web pages but without formal application program interfaces (APIs).  These
tools are build on the Python
...
serve their own data.  Examples include SDSS, 2MASS, UKIDSS, VSA.
...
custom websites, too.  These services do not follow any particular standard and can
...
some, it is not practical or even possible.  For example, the atomic and molecular databases refer
...
component of the \astropy Project ecosystem \citep{Astropy-Collaboration2013}.
