...
The Virtual Observatory (VO) has some goals similar to \texttt{astroquery},
though their approach and philosophy is different.  Where VO services provide a
single point of access for all VO-compatible services, \texttt{astroquery}
provides a collection of access points that do not require a specific API from
the hosting service.  The general philosophy in \texttt{astroquery} is to
...
more difficult, it keeps the barrier to entry for new users fairly low.
